...
for i in range(n):
    for j in range(n):
        g_inp[i, j] = np.exp(-((i - n / 2) ** 2 + (j - n / 2)
                               ** 2) / (2. * sigma_inp * sigma_inp))
...
def Crop(img, center, scale, rot, res):
    ...
    pad = int(np.math.ceil(
        (((ul - br) ** 2).sum() ** 0.5) / 2 - (br[0] - ul[0]) / 2))
    if rot != 0:
        ul = ul - pad
        br = br + pad
    ...
    try:
        newImg[new_[0]:new_[1], new_[2]:new_[3],
               :] = tmpImg[old_[0]:old_[1], old_[2]:old_[3], :]
    except:
    ...
    if rot != 0:
        M = cv2.getRotationMatrix2D(
            (newImg.shape[0] / 2, newImg.shape[1] / 2), rot, 1)
...
def DrawGaussian(img, pt, sigma, truesigma=-1):
    ...
    ul = [int(np.math.floor(pt[0] - tmpSize)),
          int(np.math.floor(pt[1] - tmpSize))]
    br = [int(np.math.floor(pt[0] + tmpSize)),
          int(np.math.floor(pt[1] + tmpSize))]
    ...
    if truesigma == 0.5:
        ...
        g[:, -1] *= 0
        g *= 1.5
    g_x = [max(0, -ul[0]), min(br[0], img.shape[1]) -
           max(0, ul[0]) + max(0, -ul[0])]
    g_y = [max(0, -ul[1]), min(br[1], img.shape[0]) -
           max(0, ul[1]) + max(0, -ul[1])]
...
def myDrawGaussian(img, pt, sigmaredundant):
    ...
    if (pt[0] < 1 or pt[1] < 1 or pt[0] > img.shape[0] or pt[1] > img.shape[1]):
        return img
    img[max(pt[0] - 3, 0):min(pt[0] + 3, img.shape[0]),
        max(pt[1] - 0, 0):min(pt[1] + 0, img.shape[1])] = 1
    img[max(pt[0] - 2, 0):min(pt[0] + 2, img.shape[0]),
        max(pt[1] - 1, 0):min(pt[1] + 1, img.shape[1])] = 1
    img[max(pt[0] - 2, 0):min(pt[0] + 2, img.shape[0]),
        max(pt[1] - 2, 0):min(pt[1] + 2, img.shape[1])] = 1
    img[max(pt[0] - 0, 0):min(pt[0] + 0, img.shape[0]),
        max(pt[1] - 3, 0):min(pt[1] + 3, img.shape[1])] = 1
    if (img.sum() == 0):
        print(pt)
        assert(img.sum() != 0)
    img = img / img.sum()
